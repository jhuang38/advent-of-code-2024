use std::{error::Error, fs::File, io::{BufRead, BufReader}, cmp::{max, min}};
...
pub fn safe_reports() -> Result<(), Box<dyn Error>> {
    ...
    for line in reader.lines() {
        let line = line?;
        let line_nums: Vec<u32> = line.split(" ").into_iter().map(|elem| {
            elem.parse::<u32>()
        }).collect::<Result<_, _>>()?;
        reports.push(line_nums);
    }
    let safe_count = reports.iter().filter(|report| {
...
pub fn safe_reports_dampened() -> Result<(), Box<dyn Error>> {
    ...
    for line in reader.lines() {
        let line = line?;
        let line_nums: Vec<i32> = line.split(" ").into_iter().map(|elem| {
            elem.parse::<i32>()
        }).collect::<Result<_, _>>()?;
        reports.push(line_nums);
    }
    let safe_count = reports.iter().filter(|nums| {
